...
const SignIn = ({ onWeb3ProviderSet, onWeb3AuthInstanceSet }: SignInProps) => {
  ...
  useEffect(() => {
    const initWeb3AuthCore = async () => {
      ...
      const openLoginAdapter = new OpenloginAdapter({ adapterSettings: { network: 'mainnet', clientId: web3AuthClientId } });
  ...
  const loginWithAdapter = async (adapter: WALLET_ADAPTER_TYPE, loginProvider?: LOGIN_PROVIDER_TYPE) => {
    ...
    if (!web3Auth) {
      setErrorMessage('Not yet initialized, please try again in a moment.');
    ...
    try {
      web3authProvider = await web3Auth.connectTo(adapter, { loginProvider });
    } catch (e) {
